function rmse = func_train_1(hyperpara_set,N,Lorenz96_m)
...
train_r_step_length = round( 2200 /reservoir_tstep );
...
for tp_i = 1:tp_train_length
    ...
    for dim_i = 1:dim_ode+3 % normalize    +3 parallel
        ts_train(:,dim_i) = (ts_train(:,dim_i) - 1) / 1.5;
    end
    ts_train = ts_train(train_r_step_cut+1:end,:); % cut
    
    ts_train = ts_train(:,[3,4,1,2,5,end]); % parallel
